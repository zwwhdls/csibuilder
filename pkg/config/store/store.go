...
func New(fs machinery.Filesystem) Store {
	return &yamlStore{fs: fs.FS}
}

// New implements store.Store interface
func (s *yamlStore) New(version config.Version, conf config.Config) error {
	s.cfg = conf
	if conf == nil {
...
func (s *yamlStore) LoadFrom(path string) error {
	...
	if err != nil {
		return config.LoadError{Err: fmt.Errorf("unable to read %q file: %w", path, err)}
	}

	// Check the file version
	var versioned versionedConfig
	if err := yaml.Unmarshal(in, &versioned); err != nil {
		return config.LoadError{Err: fmt.Errorf("unable to determine config version: %w", err)}
	}
	...
	if err != nil {
		return config.LoadError{Err: fmt.Errorf("unable to create config for version %q: %w", versioned.Version, err)}
	}

	// Unmarshal the file content
	if err := cfg.UnmarshalYAML(in); err != nil {
		return config.LoadError{Err: fmt.Errorf("unable to unmarshal config at %q: %w", path, err)}
	}
...
func (s *yamlStore) SaveTo(path string) error {
	...
	if s.cfg == nil {
		return config.SaveError{Err: fmt.Errorf("undefined config, use one of the initializers: New, Load, LoadFrom")}
	}
	...
	if s.mustNotExist {
		...
		if os.IsNotExist(err) {
			...
			return config.SaveError{Err: fmt.Errorf("configuration already exists in %q", path)}
		} else {
			return config.SaveError{Err: fmt.Errorf("unable to check for file prior existence: %w", err)}
		}
	}
	...
	if err != nil {
		return config.SaveError{Err: fmt.Errorf("unable to marshal to YAML: %w", err)}
	}

	// Write the marshalled configuration
	err = afero.WriteFile(s.fs, path, content, 0600)
	if err != nil {
		return config.SaveError{Err: fmt.Errorf("failed to save configuration to %q: %w", path, err)}
	}
